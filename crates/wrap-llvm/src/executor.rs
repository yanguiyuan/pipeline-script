...
impl JITExecutor {
    pub(crate) fn new(module:Arc<LLVMModuleRef>,executor:LLVMExecutionEngineRef,func_map:HashMap<String,Function>)->Self{
        Self{module,inner:executor,function_map:func_map}
    }
    ...
    pub fn run_function(&self, name:impl AsRef<str>, args: &mut [LLVMGenericValueRef])->LLVMGenericValueRef{
        ...
        let result =  unsafe { LLVMRunFunction(self.inner, f, args.len() as c_uint, args.as_mut_ptr()) };
        result
    }
}