...
impl Builder{
    ...
    pub fn build_array(&self,el_ty:LLVMType,arr:Vec<LLVMValue>)->LLVMValue{
        ...
        for i in 0..arr.len(){
            ...
            let gep = unsafe{LLVMBuildGEP2(self.inner,el_ty.as_llvm_type_ref(),arr0,&mut index.as_llvm_value_ref() as *mut _,1,name.as_ptr())};
            unsafe {
                LLVMBuildStore(self.inner, value.as_llvm_value_ref(),gep)
            };
        }
        arr0.into()
    }
    ...
    pub fn build_store(&self,ptr:LLVMValue,val:LLVMValue){
        unsafe {
            LLVMBuildStore(self.inner, val.as_llvm_value_ref(),ptr.as_llvm_value_ref())
        };
    }
    pub fn build_load(&self,ty:LLVMType,ptr:LLVMValue)->LLVMValue{
        let name = CString::new("").unwrap();
        unsafe {
            LLVMBuildLoad2(self.inner,ty.as_llvm_type_ref(),ptr.as_llvm_value_ref(),name.as_ptr())
        }.into()
    }
    pub fn build_array_get(&self,ty:LLVMType,ptr:LLVMValue,index:LLVMValue)->LLVMValue{
        ...
        let r = unsafe {
            let ptr0 = LLVMBuildGEP2(self.inner,ty.as_llvm_type_ref(),ptr.as_llvm_value_ref(),indices.as_mut_ptr(),1,name.as_ptr());
    ...
    pub fn build_array_gep(&self,ty:LLVMType,ptr:LLVMValue,index:LLVMValue)->LLVMValue{
        ...
        let mut indices = [index.as_llvm_value_ref()];
        let r = unsafe {
           LLVMBuildGEP2(self.inner,ty.as_llvm_type_ref(),ptr.as_llvm_value_ref(),indices.as_mut_ptr(),1,name.as_ptr())
        };
        r.into()
    }
    ...
    pub fn build_cond_br(&self,condition:LLVMValue,then_block:LLVMBasicBlockRef,else_block:LLVMBasicBlockRef){
        unsafe{
            LLVMBuildCondBr(self.inner,condition.as_llvm_value_ref(),then_block,else_block);
        }
    }

    pub fn build_add(&self,l:LLVMValue,r:LLVMValue)->LLVMValue{
        let name = CString::new("").unwrap();
        let r =unsafe { LLVMBuildAdd(self.inner, l.as_llvm_value_ref(), r.as_llvm_value_ref(), name.as_ptr()) };
        r.into()
    }
    pub fn build_fadd(&self,l:LLVMValue,r:LLVMValue)->LLVMValueRef{
        let name = CString::new("").unwrap();
        unsafe { LLVMBuildFAdd(self.inner, l.as_llvm_value_ref(), r.as_llvm_value_ref(), name.as_ptr()) }
    }
    pub fn build_mul(&self,l:LLVMValue,r:LLVMValue)->LLVMValue{
        let name = CString::new("").unwrap();
        let r =unsafe { LLVMBuildMul(self.inner, l.as_llvm_value_ref(), r.as_llvm_value_ref(), name.as_ptr()) };
        r.into()
    }
    pub fn build_sub(&self,l:LLVMValue,r:LLVMValue)->LLVMValue{
        let name = CString::new("").unwrap();
        let r = unsafe { LLVMBuildSub(self.inner, l.as_llvm_value_ref(), r.as_llvm_value_ref(), name.as_ptr()) };
        r.into()
    }
    ...
    pub fn build_eq(&self,l:LLVMValue,r:LLVMValue)->LLVMValue{
        let name = CString::new("").unwrap();
        unsafe { LLVMBuildICmp(self.inner,LLVMIntEQ, l.as_llvm_value_ref(), r.as_llvm_value_ref(), name.as_ptr()) }.into()
    }
    pub fn build_neq(&self,l:LLVMValue,r:LLVMValue)->LLVMValue{
        let name = CString::new("").unwrap();
        unsafe { LLVMBuildICmp(self.inner,LLVMIntNE, l.as_llvm_value_ref(), r.as_llvm_value_ref(), name.as_ptr()) }.into()
    }
    pub fn build_greater(&self,l:LLVMValue,r:LLVMValue)->LLVMValue{
        let name = CString::new("").unwrap();
        unsafe { LLVMBuildICmp(self.inner,LLVMIntSGT, l.as_llvm_value_ref(), r.as_llvm_value_ref(), name.as_ptr()) }.into()
    }
    pub fn build_less(&self,l:LLVMValue,r:LLVMValue)->LLVMValue{
        let name = CString::new("").unwrap();
        unsafe { LLVMBuildICmp(self.inner,LLVMIntSLT, l.as_llvm_value_ref(), r.as_llvm_value_ref(), name.as_ptr()) }.into()
    }
    pub fn build_call(&self,function:&Function,params:&mut [LLVMValue],var_name:impl AsRef<str>)->LLVMValue{
        let dec = function.get_declaration().as_llvm_type_ref();
        let var_name = CString::new(var_name.as_ref()).unwrap();
        let mut params = params.iter().map(|e|e.as_llvm_value_ref()).collect::<Vec<LLVMValueRef>>();
        let call_res = unsafe { LLVMBuildCall2(self.inner, dec, function.get_function_ref(), params.as_mut_ptr(), params.len() as c_uint, var_name.as_ptr()) };
        call_res.into()
    }
    pub fn build_zext(&self,value:LLVMValue,ty:LLVMType)->LLVMValue{
        let name = CString::new("").unwrap();
        unsafe { LLVMBuildZExt(self.inner, value.as_llvm_value_ref(), ty.as_llvm_type_ref(), name.as_ptr()) }.into()
    }
}
